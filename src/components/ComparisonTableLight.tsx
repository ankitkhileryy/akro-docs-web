...
const langs = [
  ...
  { key: 'js',     label: 'JS',     color: '#d4a017', accent: false },
  { key: 'go',     label: 'Go',     color: '#5ba85a', accent: false },
]

export default function ComparisonTableLight() {
    ...
    <div ref={ref} className="w-full">
      ...
      <div className="grid grid-cols-4 gap-3 mb-10">
        {scores.map((l, li) => {
          ...
          return (
            <div key={l.key} className={`p-4 rounded-xl border ${l.accent ? 'border-[#7c6af7]/30 bg-[#7c6af7]/5' : 'border-[#e5e5e5] bg-white'}`}>
              <div className="flex items-center justify-between mb-2">
                <span className={`text-xs font-mono font-semibold ${l.accent ? 'text-[#7c6af7]' : 'text-[#666]'}`}>
                  {l.accent && <Zap size={10} className="inline mr-1" fill="#7c6af7" />}
                  {l.label}
                </span>
                <span className="text-xs font-mono text-[#aaa]">{l.score}/{l.total}</span>
              </div>
              {/* Bar */}
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-500 ${
                isAnimated
                  ? 'opacity-100 translate-y-0 border-[#e5e5e5] bg-white'
                  : 'opacity-0 translate-y-3 border-transparent bg-transparent'
              }`}
              style={{ transitionDelay: `${i * 60}ms` }}
            >
              {/* Feature name */}
              <span className="flex-1 text-sm font-mono text-[#666] min-w-0 truncate">
                {row.feature}
              </span>
                ...
                return (
                  ...
                  >
                    {val ? (
                      ...
                      <div className="flex items-center justify-center w-6 h-6 rounded-full bg-[#f5f5f5] border border-[#e5e5e5]">
      ...
      <div className="flex items-center gap-3 px-4 mt-3">
        ...
        {langs.map(l => (
          <div key={l.key} className="w-16 text-center">
            <span className={`text-xs font-mono ${l.accent ? 'text-[#7c6af7]' : 'text-[#999]'}`}>
              {l.label}
            </span>
          </div>
        ))}
